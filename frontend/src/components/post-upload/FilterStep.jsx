...
const FilterStep = ({ image, selectedFilter, onFilterSelect, onNext, onBack, isVideo }) => {
  return (
    <div className="flex flex-col h-[500px] bg-[#FAF6F0] overflow-hidden rounded-b-xl border-t border-gray-300">
      <div className="relative flex-1 bg-[#FAF6F0] flex items-center justify-center overflow-hidden">
        {isVideo ? (
          <video 
            src={image} 
      ...
      <div className="p-4 bg-[#FAF6F0] border-t border-gray-300">
        <div className="flex items-center justify-between mb-4 px-1">
          <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Filters</span>
          <button 
            onClick={() => onFilterSelect("contrast(1.15) brightness(1.05) saturate(1.25) sepia(0.05)")}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 rounded-full text-[10px] font-bold text-[#1A1A1A] transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
          >
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {FILTERS.map((f) => (
            ...
            >
              ...
              <span className={`text-[10px] font-bold ${selectedFilter === f.filter ? 'text-[#0095F6]' : 'text-gray-600'}`}>
                {f.name.toUpperCase()}
              </span>
            </div>
          ))}
