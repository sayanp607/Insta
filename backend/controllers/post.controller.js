...
export const addComment = async (req, res) => {
  try {
    ...
    if (postAuthor !== commenterIdStr) {
      const userDetails = await User.findById(commentKrneWalaUserKiId).select('username profilePicture');
      const notification = {
        type: 'comment',
      ...
      console.log('Post author socket ID:', postAuthorSocketId);
      
      if (postAuthorSocketId) {
        io.to(postAuthorSocketId).emit('notification', notification);
        console.log('Notification sent to:', postAuthor);
      } else {
        console.log('Post author is not online');
      }
    } else {
